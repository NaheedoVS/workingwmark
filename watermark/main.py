...
async def worker(user_id: int):
    ...
    while sess.queue:
        ...
        # Send directly if ≤45 MB
        if os.path.getsize(out_path) <= 45 * 1024 * 1024:
            caption = f"Watermark: {text}\nResolution: 480p"
            if ftype == "photo":
                await app.send_photo(user_id, out_path, caption=caption)
            else:
                await app.send_video(user_id, out_path, caption=caption)
            os.remove(out_path)
        else:
            # Split & send as album (Telegram shows Merge button)
            await app.send_message(user_id, "Large file → splitting (Telegram will show Merge button)")
            split_dir = f"/tmp/split_{user_id}_{int(time.time())}"
            os.makedirs(split_dir, exist_ok=True)

            subprocess.run([
                "ffmpeg", "-y", "-i", out_path,
                "-c", "copy", "-map", "0", "-f", "segment",
                "-segment_time", "280", "-reset_timestamps", "1",
                f"{split_dir}/part_%03d.mp4"
            ], stdout=subprocess.DEVNULL)

            parts = sorted(Path(split_dir).glob("part_*.mp4"))
            media = []
            for i, p in enumerate(parts):
                cap = f"Watermark: {text} • Part {i+1}/{len(parts)}\n⋮ → Merge when all arrive" if i == 0 else ""
                media.append(types.InputMediaVideo(str(p), caption=cap))
            await app.send_media_group(user_id, media)
            await app.send_message(user_id, "All parts sent!\nMobile: ⋮ → Merge\nDesktop: right-click → Merge Videos")

            # Cleanup
            [p.unlink(missing_ok=True) for p in parts]
            os.rmdir(split_dir, ignore_errors=True)
            os.remove(out_path)
...
async def media(client, m):
    ...
    path = await client.download_media(m, progress=Progress(prog))
    await prog.delete()
...
async def merge(_, m):
    await m.reply(
        ...
        "Or run this command:\n"
        "```bash\n"
        "ffmpeg -f concat -safe 0 -i <(ls part_*.mp4 | sort | sed \"s/^/file '/; s/$/'/\") -c copy merged.mp4\n"
        "```",
